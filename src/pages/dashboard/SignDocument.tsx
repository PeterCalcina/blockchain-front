...
export default function SignDocument() {
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    const stats = JSON.parse(
      localStorage.getItem("pdfStats") ||
        '{"uploaded": 0, "verified": 0, "failed": 0}'
    );
    stats.uploaded += 1;
    stats.verified += Math.random() > 0.3 ? 1 : 0;
    stats.failed += Math.random() > 0.7 ? 1 : 0;
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-50 to-teal-50/30">
      ...
      <main className="flex-1 p-6">
        <div className="max-w-4xl mx-auto space-y-6">
          {/* {uploadSuccess && (
            <Alert className="border-teal-200 bg-gradient-to-r from-teal-50 to-emerald-50 shadow-md">
              <CheckCircle className="h-4 w-4 text-teal-600" />
              <AlertDescription className="text-teal-800 font-medium">
                ¡Documento subido exitosamente y listo para firma blockchain!
              </AlertDescription>
            </Alert>
          )} */}
            ...
            <Card.Root className="shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              <Card.Header className="bg-gradient-to-r from-teal-50 to-[var(--navy-50)] rounded-t-lg">
                <Card.Title className="flex items-center gap-3 text-[var(--navy-700)]">
                  ...
                  Cargar Documento para Firma Digital
                </Card.Title>
                <Card.Description className="text-gray-600">
                  Sube tu documento PDF para firmarlo digitalmente en la
                  blockchain de forma segura
                </Card.Description>
              </Card.Header>
                  ...
                  {isUploading ? (
                    <div className="space-y-6">
                      <div className="relative">
                        <div className="animate-spin rounded-full h-16 w-16 border-4 border-teal-200 border-t-[var(--navy-600)] mx-auto"></div>
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="w-8 h-8 bg-gradient-to-br from-teal-500 to-[var(--navy-600)] rounded-full"></div>
                        </div>
                      </div>
                      <p className="text-[var(--navy-600)] font-medium text-lg">
                        Procesando documento...
                      </p>
                      <p className="text-gray-500">
                        Preparando para firma blockchain
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">
                          o arrastra y suéltalo para firmar digitalmente
                        </p>
                      </div>
                      <input
                        type="file"
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-gradient-to-r from-[var(--navy-50)] to-teal-50 p-6 rounded-xl border border-gray-200">
                        <div className="space-y-3">
                          ...
                          >
                            Nombre del Registro Blockchain
                          </label>
                          <input
                            id="fileName"
                            type="text"
                            placeholder="Ej: Contrato_Servicios_2024"
                            value={fileName}
                            onChange={(e) => setFileName(e.target.value)}
                            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 bg-white text-[var(--navy-700)] placeholder-gray-400 transition-colors duration-200"
                          />
                      ...
                      <div className="flex justify-center pt-2">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            alert(
                              `Iniciando proceso de firma en blockchain para: "${fileName}"`
                            );
                          }}
                        >
                          <Shield className="h-5 w-5 mr-2" />
                          Firmar Documento en la Blockchain
                        </Button>
                      </div>
                    </div>
                  )}
